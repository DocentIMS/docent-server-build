...
# Docent branding: the medialog.notifications add-on creates a 'notifications'
# folder + 'notifications-collection' Collection, both titled "Notifications".
# Docent presents this feature as "Alerts", so retitle both (idempotently)
# after activation. The URL ids are left as-is so existing links keep working.
# Override the label with NOTIFICATIONS_LABEL in tenant.local if needed.
NOTIFICATIONS_LABEL="${NOTIFICATIONS_LABEL:-Alerts}"
...
# Re-apply defaults in case tenant.local set them.
PLONE_THEME="${PLONE_THEME:-docent-ims-theme}"
NOTIFICATIONS_LABEL="${NOTIFICATIONS_LABEL:-Alerts}"

# Seeded test/dev accounts to DELETE after activation. Some add-ons create demo
# users on install (e.g. DocentIMS.dashboard creates vbauser@docentims.com);
# these must not ship on client sites. Space-separated; override with
# PLONE_REMOVE_USERS in tenant.local (set to a single space to disable removal).
REMOVE_USERS="${PLONE_REMOVE_USERS:-vbauser@docentims.com docent-tester dummyuser@docentims.com}"
...
THEME_SCRIPT="/tmp/phase7e-theme.py"
RENAME_SCRIPT="/tmp/phase7e-rename.py"
REMOVE_USERS_SCRIPT="/tmp/phase7e-remove-users.py"
...
# NOTE: theme activation is deliberately NOT done here. Using the
# quickinstaller (get_installer) above taints this process so that
# plone.app.theming's getAvailableThemes()/getTheme() return garbage in the
# same run (the theme appears "not found"). The theme is therefore activated
# by a SEPARATE bin/instance run (PHASE7E_THEME_SCRIPT) that never touches the
# installer - see step 3 in the bash wrapper.
...
# Theme activation runs in its OWN bin/instance run (a clean process). It must
# not share a process with the quickinstaller above: using get_installer taints
# the process so plone.app.theming's getTheme()/getAvailableThemes() stop
# returning the real themes (they come back as "not found"). A fresh process
# resolves the theme correctly - confirmed against the live site.
cat > "$THEME_SCRIPT" <<'PYEOF'
# phase7e-marker - generated by phase7e-plone-activate.sh (theme step)
#
# Run via: bin/instance run /tmp/phase7e-theme.py
# Reads env vars:
#   PHASE7E_SITE_ID  - e.g. "Plone"
#   PHASE7E_THEME    - registered theme name to activate (may be empty)
#
import os
import sys
import transaction
...
if not THEME:
    print("THEME no PLONE_THEME set; skipping theme activation")
    sys.exit(0)

if SITE_ID not in app.objectIds():
    print("ERROR: Plone site '/%s' not found." % SITE_ID)
    sys.exit(1)
site = app[SITE_ID]
...
from plone.app.theming.utils import applyTheme, getTheme, getAvailableThemes
...
if theme is not None:
    ...
    # Themes register their id under __name__ (name is None); list them so the
    # operator can correct PLONE_THEME. Not a failure - theme is best-effort.
    names = ", ".join(
        (getattr(t, "__name__", None) or getattr(t, "title", None) or repr(t))
...
sys.exit(0)
PYEOF
chown "$PLONE_USER:$PLONE_USER" "$THEME_SCRIPT"
chmod 600 "$THEME_SCRIPT"

if [ ! -s "$THEME_SCRIPT" ]; then
    log_fail "Heredoc wrote zero bytes to $THEME_SCRIPT"
    exit 1
fi
log_done "Wrote $THEME_SCRIPT"

# Content branding: retitle the notifications folder + collection to the Docent
# label ("Alerts"). Idempotent - only writes when the title differs. Runs in its
# own bin/instance run; does not touch the installer.
cat > "$RENAME_SCRIPT" <<'PYEOF'
# phase7e-marker - generated by phase7e-plone-activate.sh (content-label step)
#
# Reads env vars:
#   PHASE7E_SITE_ID  - e.g. "Plone"
#   PHASE7E_LABEL    - new title for the notifications folder + collection
#
import os
import sys
import transaction
...
SITE_ID = os.environ["PHASE7E_SITE_ID"]
LABEL = os.environ.get("PHASE7E_LABEL", "").strip()

if not LABEL:
    print("LABEL empty; skipping notifications->label rename")
    sys.exit(0)
...
folder = site.get("notifications")
if folder is None:
    print("LABEL no 'notifications' folder yet; nothing to rename")
    sys.exit(0)
...
coll = folder.get("notifications-collection")
if coll is not None and coll.Title() != LABEL:
    coll.setTitle(LABEL)
    coll.reindexObject()
    changed.append("notifications-collection")

transaction.commit()
print("LABEL retitled to %r: %s" % (LABEL, ", ".join(changed) if changed else "nothing (already set)"))
sys.exit(0)
PYEOF
chown "$PLONE_USER:$PLONE_USER" "$RENAME_SCRIPT"
chmod 600 "$RENAME_SCRIPT"

if [ ! -s "$RENAME_SCRIPT" ]; then
    log_fail "Heredoc wrote zero bytes to $RENAME_SCRIPT"
    exit 1
fi
log_done "Wrote $RENAME_SCRIPT"

# Cleanup: delete seeded test/dev accounts that add-ons create on install
# (e.g. DocentIMS.dashboard's vbauser@docentims.com). Own clean bin/instance
# run, AFTER activation so it catches users created during it.
cat > "$REMOVE_USERS_SCRIPT" <<'PYEOF'
# phase7e-marker - generated by phase7e-plone-activate.sh (remove-test-users step)
#
# Reads env vars:
#   PHASE7E_SITE_ID       - e.g. "Plone"
#   PHASE7E_REMOVE_USERS  - space-separated usernames to delete
#
import os
import sys
import transaction
...
SITE_ID = os.environ["PHASE7E_SITE_ID"]
USERS = os.environ.get("PHASE7E_REMOVE_USERS", "").split()

if not USERS:
    print("REMOVE no users configured; skipping")
    sys.exit(0)
...
for name in USERS:
    try:
        if plone.api.user.get(username=name) is None:
            print("absent  %s" % name)
            continue
        plone.api.user.delete(username=name)
        removed.append(name)
...
sys.exit(0)
PYEOF
chown "$PLONE_USER:$PLONE_USER" "$REMOVE_USERS_SCRIPT"
chmod 600 "$REMOVE_USERS_SCRIPT"

if [ ! -s "$REMOVE_USERS_SCRIPT" ]; then
    log_fail "Heredoc wrote zero bytes to $REMOVE_USERS_SCRIPT"
    exit 1
fi
log_done "Wrote $REMOVE_USERS_SCRIPT"
...
# Theme activation in a SEPARATE, clean bin/instance run (see note above). The
# service is still stopped, so the ZODB lock is free. Best-effort: a missing
# theme name warns but does not fail the phase.
set +e
sudo -u "$PLONE_USER" \
    PHASE7E_SITE_ID="$PLONE_SITE_ID" \
    PHASE7E_THEME="$PLONE_THEME" \
    bash -c "cd '$PLONE_INSTANCE_DIR' && bin/instance run '$THEME_SCRIPT'"
THEME_RC=$?
set -e
if [ "$THEME_RC" -eq 0 ]; then
    log_done "Theme step completed (PLONE_THEME=$PLONE_THEME)"
else
    log_warn "Theme step exited $THEME_RC (add-ons are unaffected)"
fi

# Content-label step (notifications -> "Alerts"), separate clean bin/instance
# run. Service is still stopped. Best-effort: a failure warns, does not fail
# the phase.
set +e
sudo -u "$PLONE_USER" \
    PHASE7E_SITE_ID="$PLONE_SITE_ID" \
    PHASE7E_LABEL="$NOTIFICATIONS_LABEL" \
    bash -c "cd '$PLONE_INSTANCE_DIR' && bin/instance run '$RENAME_SCRIPT'"
RENAME_RC=$?
set -e
if [ "$RENAME_RC" -eq 0 ]; then
    log_done "Content-label step completed (notifications -> $NOTIFICATIONS_LABEL)"
else
    log_warn "Content-label step exited $RENAME_RC (add-ons are unaffected)"
fi

# Remove seeded test/dev accounts (after activation, so it catches users created
# during it). Separate clean bin/instance run; service still stopped.
# Best-effort: a failure warns, does not fail the phase.
set +e
sudo -u "$PLONE_USER" \
    PHASE7E_SITE_ID="$PLONE_SITE_ID" \
    PHASE7E_REMOVE_USERS="$REMOVE_USERS" \
    bash -c "cd '$PLONE_INSTANCE_DIR' && bin/instance run '$REMOVE_USERS_SCRIPT'"
REMOVE_RC=$?
set -e
if [ "$REMOVE_RC" -eq 0 ]; then
    log_done "Removed seeded test users if present ($REMOVE_USERS)"
else
    log_warn "Remove-test-users step exited $REMOVE_RC (add-ons are unaffected)"
fi
