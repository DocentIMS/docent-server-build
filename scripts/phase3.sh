...
#
# Passwords for the MariaDB root user are NOT printed by this script. They
# are in CREDENTIALS.txt (generated by phase0-bootstrap.sh) at the repo root.
#
...
if [ -f "$ROOT_DEFAULTS_FILE" ] && grep -q "phase3-marker" "$ROOT_DEFAULTS_FILE"; then
    ...
    # Use ROOT_DB_PW from secrets.local if available, otherwise generate.
