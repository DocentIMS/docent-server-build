...
if grep -q "^PLONE_ADMIN_PW=..*" "$CREDENTIALS_FILE" 2>/dev/null; then
    # Already filled in (re-run case).
    log_skip "PLONE_ADMIN_PW already recorded in CREDENTIALS.txt"
elif grep -q "^PLONE_ADMIN_PW=$" "$CREDENTIALS_FILE" 2>/dev/null; then
    # Phase 0 left empty placeholder lines in the "Plone Site - ADD THE TEAM
    # PLONE SITE" section. Fill them in (use | as sed delimiter so / in paths
    # doesn't break the substitution).
    sed -i \
        -e "s|^PLONE_ADMIN_PW=$|PLONE_ADMIN_PW=$PLONE_ADMIN_PW|" \
        -e "s|^PLONE_VERSION=$|PLONE_VERSION=$PLONE_VERSION|" \
        -e "s|^PLONE_INSTANCE_DIR=$|PLONE_INSTANCE_DIR=$PLONE_INSTANCE_DIR|" \
        "$CREDENTIALS_FILE"
    chmod 600 "$CREDENTIALS_FILE"
    log_done "Filled Plone credentials into $CREDENTIALS_FILE"
else
    # Legacy fallback: CREDENTIALS.txt was generated by a phase 0 that
    # predates the in-place Plone section. Append at the end so re-running
    # phase 7b still records the credentials.
    {
        echo ""
        echo "=============================================================="
        echo "  Plone Site - ADD THE TEAM PLONE SITE"
        echo "=============================================================="
        echo ""
    ...
    chmod 600 "$CREDENTIALS_FILE"
    log_done "Recorded Plone admin credentials to $CREDENTIALS_FILE (appended)"
fi
